...
import { importSourcesFromOpenLibrary } from "@/lib/openlibrary/sourceImport"
...
export async function POST(req: Request) {
  ...
  let works: Awaited<ReturnType<typeof importSourcesFromOpenLibrary>>
  try {
    works = await importSourcesFromOpenLibrary({
      authorName,
      title,
      isbn,
      limit,
    ...
    const message = error instanceof Error ? error.message : "Open Library import failed"
